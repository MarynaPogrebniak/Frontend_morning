...
const Counter1: React.FC = () => {
    ...
    useEffect(() => {
        ...
        return () => {
           console.log("Компонент будет удален"); // Аналог componentWillUnmount() в classComponent
        }
    }, []);

    useEffect(() => {
        console.log('Компонент обновляется'); // Аналог componentDidUpdate() в classComponent
